...
export default function PromotionForm({ companyId, onSubmit }) {
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onSuccess: () => {
      queryClient.setQueriesData(['promotions', companyId], (oldData) => {
        if (oldData) {
          return oldData.concat(data);
        }
      });

      queryClient.setQueriesData(['promotions'], (oldData) => {
        if (oldData) {
          return oldData.concat(data);
        }
      });
